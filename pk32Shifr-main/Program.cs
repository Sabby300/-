...
string sql1 = "SELECT word, sdvig FROM words";
using var cmd1 = new MySqlCommand(sql1,con);

while (rdr.Read())
{
    string res = Decode(rdr.GetString(0).rdr.GetInt32(1));
    Console.WriteLine("Расшифрованное слово:{0}",res);
}
...
//
string Decode(string_shifr, int _sdvig)
{
    string alfavit = "абвгдеёжзийклмнопрстуфхцчшщъыьэюя"; //исходный алфавит
    ...
    {
            int letterNumber = newlfavit.IndexOf(userInput[i]);
